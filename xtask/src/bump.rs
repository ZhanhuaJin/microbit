...
pub fn bump_versions(new_version: &str, dry_run: bool) {
    let common_toml_path = "microbit-lib/Cargo.toml";
    ...
    {
        println!("microbit-lib: {} -> {}", old_version, new_version);

        // Bump `microbit-lib`'s version.
        let from = format!(r#"version = "{}""#, old_version);
        let to = format!(r#"version = "{}""#, new_version);
        file_replace("microbit-lib/Cargo.toml", &from, &to, dry_run);
        ...
        let librs_path = "microbit-lib/src/lib.rs";
    ...
    for (crate_name, _, _) in CRATES {
        ...
        // Bump the crate's dependency on `microbit-lib`.
        let from = format!(r#"version = "={}""#, old_version);
        let to = format!(r#"version = "={}""#, new_version);
        file_replace(&toml_path, &from, &to, dry_run);
